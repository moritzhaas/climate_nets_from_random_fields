...
for nu in nus:
    for len_scale in len_scales:
        ...
        outfilename = f'othertelestats_part{irun}_{distrib}_{corr_method}_{typ}_w{weighted}_nu{nu}_len{len_scale}_ar{ar}_{grid_type}{n_lat}_time{n_time}_{num_runs}_tol{robust_tolerance}_eps{eps2}.txt'
        #all_tele1 = np.zeros((numth))
        all_tele2 = np.zeros((numth))
        all_fdr = np.zeros((numth))
        #all_tele3 = np.zeros((numth, num_runs))
        all_countrobusttele2 = np.zeros(numth)
        all_countrobusttele2raw = np.zeros((numth))
        all_counttele2 = np.zeros((numth))
        ...
        for i, density in enumerate(denslist):
            ...
            all_tele2[i] = teleconn(adj, dists, eps2, tolerance = 0.1)
            #all_tele3[i] = teleconn(adj, dists, eps3)
            
            print('Computing count robust tele. time since previous print: ',irun,i, time.time()-curr_time)
            curr_time = time.time()

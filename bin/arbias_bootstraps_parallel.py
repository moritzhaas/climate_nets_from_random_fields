...
empdatas = find(f'*_fixedcoeff_nu{nu}_len{len_scale}_ar{ac}_ac2{ac2}_{grid_type}{n_lat}_time{n_time}_*',base_path+'empdata/')
if empdatas == []:
    seed = int(time.time())
    np.random.seed(seed)
    orig_data = diag_var_process(ar_coeff, cov, n_time)
    mysave(base_path+'empdata/',f'arbiasdata_fixedcoeff_nu{nu}_len{len_scale}_ar{ac}_ac2{ac2}_{grid_type}{n_lat}_time{n_time}_{seed}.txt',orig_data)    
else:
...
mysave(base_path+'signif/',f'arbiasquants_iaaft_part{irun}_nu{nu}_len{len_scale}_{ac}_{ac2}_time{n_time}.txt',[quants, mean_std ,data, alphas])

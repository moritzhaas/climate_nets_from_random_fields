...
for ty in ['threshold', 'knn']:
    if robust_tolerance is None:
        ...
        truestats_file = f'truegraphstats_matern{nu}_{len_scale}_{ty}_{len(ks)}_ranks{ranks}.txt'
        ...
        # add more tele?
        for w in [True, False]:
            truestats_file = f'truetelestats_matern{nu}_{len_scale}_{ty}_{len(ks)}_tol{robust_tolerance}_eps{eps2}_w{w}_ranks{ranks}.txt'
            if not os.path.exists(base_path + truestats_file):
                for i in range(len(ks)):
                    if ty == 'knn':
                        k = ks[i]
